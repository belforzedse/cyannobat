...
const baseChipClasses = [
  ...
  'transition-[background-color,border-color,color,box-shadow,transform] duration-300 ease-glass',
].join(' ');

const toneClasses: Record<GlassChipTone, string> = {
  default: '',
  muted:
    'bg-[color-mix(in_srgb,var(--card)_70%,transparent)] text-[color:var(--muted-foreground)] dark:bg-[color-mix(in_srgb,var(--card)_50%,transparent)]',
  current: [
    'border-accent/42',
...
export const GlassChip = React.forwardRef<HTMLElement, GlassChipProps>(
  ({ as, tone, shape, interactive, className, children, ...props }, ref) => {
