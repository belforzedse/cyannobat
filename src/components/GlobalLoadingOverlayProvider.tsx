...
const GlobalLoadingOverlayContext = createContext<GlobalLoadingOverlayContextValue | undefined>(undefined)
...
export const GlobalLoadingOverlayProvider = ({ children, defaultMessage = 'در حال بارگذاری...' }: GlobalLoadingOverlayProviderProps) => {
  ...
  const latestMessage = activeRequests.length > 0 ? activeRequests[activeRequests.length - 1]?.message : undefined
  ...
  return (
    <GlobalLoadingOverlayContext.Provider value={contextValue}>
      {children}
      <GlobalLoadingOverlay active={activeRequests.length > 0} message={latestMessage ?? defaultMessage} />
