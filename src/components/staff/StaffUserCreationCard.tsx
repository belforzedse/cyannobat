...
} from '@/lib/staff/rolePermissions';
import {
...
const roleHelpText: Record<AssignableRole, string> = {
  patient:
    'حساب‌های بیمار فقط به داشبورد شخصی دسترسی دارند و نمی‌توانند کاربران دیگر را مدیریت کنند.',
...
const StaffUserCreationCard = ({ currentUser }: StaffUserCreationCardProps) => {
  const creatableRoles = useMemo(
    () => getCreatableRolesForUser(currentUser.roles),
    [currentUser.roles],
  );
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.error(error);
      const description =
        'در ایجاد کاربر مشکلی پیش آمد. لطفاً اتصال خود را بررسی کنید و دوباره تلاش کنید.';
      setFormError(description);
      showToast({ description, variant: 'error' });
    } finally {
    ...
    <Card variant="default" padding="lg" className="flex flex-col gap-6">
      <div className="flex flex-col gap-1 text-right">
        ...
        <p className="text-xs text-muted-foreground">
          برای افزودن کاربر جدید، نقش مناسب را انتخاب کنید و اطلاعات تماس او را وارد کنید. این
          اطلاعات فقط برای تیم شما قابل مشاهده است.
        </p>
      </div>
      ...
      <form className="flex flex-col gap-5" onSubmit={handleSubmit}>
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-[11px] text-muted-foreground">
            رمز عبور را از طریق کانال‌های امن در اختیار کاربر بگذارید. در صورت نیاز می‌توانید بعداً
            نقش او را نیز تغییر دهید.
          </p>
