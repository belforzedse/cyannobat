...
const sendTicketEmail = async (
  ...
  const to = process.env.SUPPORT_EMAIL_TO || process.env.SUPPORT_EMAIL_RECIPIENT || 'support@cyannobat.com'
  const from = process.env.SUPPORT_EMAIL_FROM || process.env.SUPPORT_EMAIL_IDENTITY || 'no-reply@cyannobat.com'
...
export const POST = async (request: Request) => {
  ...
  try {
    ...
    if (deliveryMode !== 'email' && (process.env.SUPPORT_EMAIL_TO || process.env.SUPPORT_EMAIL_RECIPIENT)) {
      try {
        await sendTicketEmail(payload, parsed.data, clientDetails)
        return NextResponse.json(
          {
