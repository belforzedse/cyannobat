...
import type { StaffProvider, StaffProviderAvailabilityWindow, StaffUser } from '@/features/staff/types'
...
vi.mock('@/components/GlobalLoadingOverlayProvider', async () => {
  const actual = await vi.importActual<
    typeof import('@/components/GlobalLoadingOverlayProvider')
...
describe('validateAvailabilityWindows', () => {
  ...
  it('returns an error message for invalid days', () => {
    expect(
      validateAvailabilityWindows([
        { ...validWindow, day: 'caturday' },
      ]),
    ).toBe('روز انتخاب شده معتبر نیست.')
  })

  it('returns an error message when times do not match the HH:MM format', () => {
    expect(
      validateAvailabilityWindows([
        { ...validWindow, startTime: '9:00', endTime: '12:00' },
      ]),
  ...
  it('returns an error message when end time is not after start time', () => {
    expect(
      validateAvailabilityWindows([
        { ...validWindow, startTime: '10:00', endTime: '09:00' },
      ]),
...
describe('useProviderAvailabilityEditor', () => {
  ...
  it('allows admins to edit any provider', () => {
    const providerB: StaffProvider = { ...baseProvider, id: 'provider-2', accountId: 'other-doctor' }
  ...
  it('stores validation errors when saving invalid availability windows', async () => {
    ...
    expect(result.current.getErrorMessage('provider-1')).toBe('زمان پایان باید بعد از زمان شروع باشد.')
  ...
  it('surfaces permission errors returned by the server', async () => {
    ...
    expect(result.current.getErrorMessage('provider-1')).toBe('اجازه ویرایش این ارائه‌دهنده را ندارید.')
